...
def comprehensive_evaluation(long_df, output_dir='data'):
    ...
    print(f"  新方法 Spearman:        {consistency_new:.4f}" if not np.isnan(consistency_new) else "  新方法: N/A (缺少 final_rank_alt)")

    if rank_table is not None:
        try:
            out_path = output_dir / 'final_rank_comparison.csv'
            rank_table.to_csv(out_path, index=False, encoding='utf-8-sig')
            print(f"\n  ✓ [已导出] 三种算法最终排名 vs 真实排名: {out_path}")
        except Exception:
            pass
    ...
    summary_data = {
        ...
        '新方法': [f'{new_flip:.6f}', f'{new_robust:.6f}', f'{consistency_new:.4f}' if not np.isnan(consistency_new) else 'N/A'],
